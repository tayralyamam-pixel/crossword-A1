...
def get_svg_string(grid, number_map, clues, grid_size, t):
    cell_size = 40
    ...
    for row in range(grid_size):
        for col in range(grid_size):
            ...
            if token != EMPTY:
                ...
                svg += f'  <text x="{x + cell_size/2}" y="{y + cell_size/2 + 4}" font-family="Arial" font-size="{font_size}" text-anchor="middle" dominant-baseline="middle" font-weight="bold">{token}</text>\n'
                
    y_clues = (grid_size * cell_size) + 30
    svg += f'  <text x="20" y="{y_clues}" font-family="Arial" font-size="18" font-weight="bold">{t["clues_title"]}</text>\n'
    
    y_clues += 25
    svg += f'  <text x="20" y="{y_clues}" font-family="Arial" font-size="14" font-weight="bold">{t["clues_across"]}</text>\n'
    for clue in clues["Across"]:
        y_clues += 20
        svg += f'  <text x="30" y="{y_clues}" font-family="Arial" font-size="13">{clue}</text>\n'
        
    y_clues += 30
    svg += f'  <text x="20" y="{y_clues}" font-family="Arial" font-size="14" font-weight="bold">{t["clues_down"]}</text>\n'
...
# ==========================================
st.set_page_config(page_title="مولّد الألغاز", page_icon="🧩", layout="wide")

# اختيار لغة الواجهة أولاً
selected_lang = st.sidebar.selectbox("🌐 لغة الواجهة / UI Language / Langue:", ["العربية 🇸🇦", "English 🇬🇧", "Français 🇫🇷"])
t = translations[selected_lang] # سحب القاموس الخاص باللغة المختارة
...
if st.button(t["btn_generate"]):
    ...
    if not data_dict:
        ...
        # تمرير قاموس اللغة (t) إلى دالة التصدير ليتم ترجمة الأسئلة داخل الـ SVG
        svg_output = get_svg_string(grid, num_map, final_clues, grid_size, t)
        ...
        st.download_button(
            ...
            data=svg_output,
            file_name="multilingual_puzzle.svg",
            mime="image/svg+xml"
        )
